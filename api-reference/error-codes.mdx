...
title: Error Codes
icon: radiation
description: Encountered an error? This page explains Hyperrail API error codes, what they mean, and how to resolve them.
---

## Hyperrail API Response Codes

Hyperrail uses standard HTTP response codes to communicate the outcome of API requests. Always inspect both the **HTTP status code** and the **response body** to fully understand what happened.
...
- **500, 502, 503, 504**  
  These indicate internal server errors on Hyperrail’s infrastructure.  
...
Beyond HTTP status codes, Hyperrail responses may include structured error objects with additional context such as error type, message, and resolution hints.
...
| **Server Error** | Internal error on Hyperrail’s side. | Retry later and monitor webhook events. |

---

By properly handling these errors and relying on webhooks for final state confirmation, you can build **resilient, fault-tolerant integrations** with the Hyperrail API.
